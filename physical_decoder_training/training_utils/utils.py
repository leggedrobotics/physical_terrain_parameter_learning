...
class RNNDataset(Dataset):
    ...
    def input_selection(self, input_data, input_type):
        if input_type == "hidden":
            ...
            raise ValueError(
                "Invalid input_type. Choose 'hidden', 'pro+exte', or 'all'"
            )
        return input_data


def save_model(
    model, model_name, use_rnn, input_width=None, use_weight=False, hyperparams=None
):
    """Save the model to a folder."""
    architecture_tag = "RNN" if use_rnn else "MLP"
    rnn_mode = (
        model.decoder.mode
        if hasattr(model, "decoder") and (hasattr(model.decoder, "mode") and use_rnn)
        else ""
    )  # Add mode only for RNN
    weight_tag = "weighted" if use_weight else ""
    current_time = datetime.datetime.now().strftime("%Y_%m_%d_%H%M")
    paradigm = (
        model.decoder.paradigm
        if hasattr(model, "decoder") and hasattr(model.decoder, "paradigm")
        else ""
    )
    if input_width:
        filename = f"model_{model_name}_{architecture_tag}_{rnn_mode}_InputWidth{input_width}_{weight_tag}_{paradigm}_{current_time}.pth".replace(
            "__", "_"
        )
    else:
        filename = f"model_{model_name}_{architecture_tag}_{rnn_mode}_{weight_tag}_{current_time}.pth".replace(
            "__", "_"
        )
...
def get_dataloader_for_env_id(
    ...
    dataset = RNNDataset(
        env_id_data, use_rnn=use_rnn, output_type=output_type, input_type=input_type
    )
...
def evaluate_on_data(
    ...
    model_name,
    use_rnn,
    reset_hidden_each_epoch,
    output_type,
    input_type,
    overlapp=None,
    bin_accuracy_calculator=None,
    logger=None,
):
    ...
    data_loader = get_dataloader_for_env_id(
        organized_data,
        env_id,
        1,
        use_rnn=use_rnn,
    ...
    with torch.no_grad():
        for batch_inputs, batch_true_values in data_loader:
            batch_inputs = batch_inputs.to("cuda:0")
            if use_rnn:
                predictions, hidden = model.decoder.get_unnormalized_recon(
                    batch_inputs, hidden
                )
                if model.decoder.paradigm == "ordinal":
                    probs = model.decoder.prob
            ...
            if model.decoder.middle_only:
                batch_true_values = batch_true_values[
                    :, model.decoder.exact_middle_step, :
                ].unsqueeze(1)
    ...
    if bin_accuracy_calculator:  # Ensure that the BinAccuracy instance is provided
        accuracies, confusion_matrix = bin_accuracy_calculator.calculate_accuracy(
            all_predictions, all_true_values, all_probs
        )
        average_accuracy = sum(accuracies) / len(accuracies)
        print(f"Average accuracy: {average_accuracy*100:.2f}%")
        metrics = {
    ...
    for i in range(4):
        ...
        axs[i].plot(
            predictions[:, i],
            label=f"Predicted {model_name} values",
            color="red",
            linestyle="--",
        )
        axs[i].legend()
        axs[i].set_title(
            f"Dimension {i+1} - True vs Predicted {model_name} values for env_id {env_id}"
        )
    plt.tight_layout()
    plt.close()
...
def collect_errors(model, loader, use_rnn, reset_hidden_each_epoch):
    ...
    for data in loader:
        ...
        errors = np.abs(
            recon.cpu().numpy() - priv.cpu().numpy()
...
def collect_errors_prob(model, loader, use_rnn, reset_hidden_each_epoch):
    ...
    return (
        np.hstack(all_true_values),
        np.hstack(all_errors),
        np.hstack(all_stds),
    )  # Stack horizontally
...
def compute_mean_errors_per_bin_prob(
    true_values, errors, stds, interval, min_range, max_range
):
    bins = np.arange(min_range, max_range + interval, interval)
    bin_means = []
    bin_stddevs = []
...
def plot_error_histogram(
    ...
    bins, bin_means, bin_stddevs, bin_errors = compute_mean_errors_per_bin(
        true_values, errors, interval, min_range, max_range
    )
    ...
    rnn_or_mlp = (
        "RNN" + "_" + rnn_mode if use_rnn else "MLP"
    )  # Modify this line to include the RNN mode
    weight_tag = "weighted" if use_weight else ""
    folder_name = "models/histograms"
    os.makedirs(folder_name, exist_ok=True)  # Create the folder if it doesn't exist
    if ratio is not None:
        filename = f"histogram_{model_name}_{rnn_or_mlp}_{input_width}_ratio{ratio}_{weight_tag}_{current_time}.png".replace(
            "__", "_"
        )
    else:
        filename = f"histogram_{model_name}_{rnn_or_mlp}_{input_width}_{weight_tag}_{current_time}.png".replace(
            "__", "_"
        )
...
def plot_error_histogram_prob(
    ...
    bins, bin_means, bin_stddevs, bin_meanstds, bin_stdstds, bin_errors = (
        compute_mean_errors_per_bin_prob(
            true_values, errors, stds, interval, min_range, max_range
        )
    )
    # Define colors for each model type
