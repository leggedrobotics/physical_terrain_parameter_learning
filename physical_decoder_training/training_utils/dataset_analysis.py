...
if __name__ == "__main__":
    ...
    filename = os.path.join(subfolder_path, timestamp + '__dataset_analysis_Version0.txt')
    base_plot_filename = os.path.join(subfolder_path, timestamp + '__dataset_analysis_Version0')
    ...
    with open(filename, 'w') as file:
        ...
        for sensor, values in combined_data.items():
            ...
            # Plotting
            fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(10, 10))  # create a 2x2 grid of subplots
            ...
            for i, foot in enumerate(feet_labels):
                for j in range(len(start_indices) - 1):
                    ...
                    axes[i].plot(np.arange(start_indices[j], start_indices[j+1]), values_np[start_indices[j]:start_indices[j+1], i], label=f'Episode {j+1}' if foot == feet_labels[0] else "", color=color)
            ...
            values = combined_data[sensor].cpu().numpy()  # Extract the values for the sensor from the combined data
            flattened_values = values.flatten()  # Flatten the values for histogram

            plt.figure(figsize=(10, 6))
            plt.hist(flattened_values, bins=10, edgecolor='black')  # Adjust 'bins' as required
        ...
        mean_length, std_length, episode_lengths_per_env=compute_episode_statistics(data)
