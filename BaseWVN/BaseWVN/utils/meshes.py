...
def make_box(length, width, height, pose=np.eye(4, dtype=np.float32), grid_size=11):
    ...
    s = 0.01
    t = 0.01
    return make_superquadric(
        length / 2, width / 2, height / 2, r, s, t, pose=pose, grid_size=grid_size
    )


def make_rounded_box(
    length, width, height, pose=np.eye(4, dtype=np.float32), grid_size=11
):
    r = 0.2
    s = 0.2
    t = 0.2
...
def make_plane(x=None, y=None, z=None, pose=np.eye(4), grid_size=10):
    ...
    if x is None:
        ...
        if x != 0:
            ...
            points = np.array([[x / 2, y / 2, 0.0], [x / 2, -y / 2, 0.0]])
    else:
    ...
    if not simple:
        ...
        if grid_size > 0:
            ...
            for i in range(4):
                for w in w_steps:
                    interp = np.expand_dims(
                        np.interp(w, [0, 1], [points[i], points[(i + 1) % 4]]), axis=0
                    )
...
def np_transform_points(pose, points):
    ...
    transformed_points = np.empty(
        (points.shape[0], pose.shape[1] - 1), dtype=np.float32
    )
