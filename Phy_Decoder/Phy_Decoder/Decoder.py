...
class OrdinalRegressionLayer(nn.Module):
    ...
    def forward(self, x, training=True):
        ...
        assert priv_dim == 4 * self.ord_num * 2, "Mismatch between priv_dim and expected dimensions"
...
class OrdinalRegressionLoss(object):
    ...
    def _create_ord_label(self, gt):
        ...
        mask = torch.linspace(0, self.ord_num - 1, self.ord_num, requires_grad=False) \
            .view(1, 1, 1, self.ord_num).to(gt.device)
...
class BeliefDecoderRNN(nn.Module):
    def __init__(
        ...
        if self.paradigm== "ordinal":
            ...
            self.ordinal_regression_loss = OrdinalRegressionLoss(ord_num=self.ord_num, model_type=model_type,discretization=self.discretization,shift_SID=self.shift)
            ...
            if hidden_to_pred_shape[-1] < priv_size or self.hidden_size < priv_size:
                raise ValueError("The final dimension of hidden_to_pred_shape or the hidden_size should be greater than or equal to priv_size.")
            
        if self.mode == "base" :
            self.hidden_to_pred = rslgym_module.MLP(
                hidden_to_pred_shape, activation_fn, hidden_size, priv_size, init_scale=1.0 / np.sqrt(2)
            )
        elif self.mode == "bi" :
            self.hidden_to_pred = rslgym_module.MLP(
                hidden_to_pred_shape, activation_fn, hidden_size*2, priv_size, init_scale=1.0 / np.sqrt(2)
            )
            ...
            self.hidden_to_gate = rslgym_module.MLP(
                hidden_to_gate_shape, activation_fn, hidden_size, inner_state_size, init_scale=1.0 / np.sqrt(2)
            )
            self.gate_to_pred = rslgym_module.MLP(
                gate_to_pred_shape, activation_fn, inner_state_size, priv_size, init_scale=1.0 / np.sqrt(2)
            )
        elif self.mode == "transformer":
            encoder_layer = TransformerDecoderLayer(d_model=encoder_input_dim, nhead=11)
            self.transformer_decoder = TransformerDecoder(encoder_layer, num_layers=num_rnn_layers)
            self.pos_encoder = PositionalEncoding(encoder_input_dim)
            self.output_layer = rslgym_module.MLP(
                hidden_to_pred_shape, activation_fn, encoder_input_dim, priv_size, init_scale=1.0 / np.sqrt(2)
            )
            pass
        ...
        # add a mode "parallel", using an independent GRU for exteroceptive input and add a gated attention layer to combine the two outputs
            ...
            if sens_size!=341 :
                raise ValueError("The sens_size should be 341 (pro+exte) for parallel mode.")
            self.rnn_pro = nn.GRU(133, self.hidden_size, self.num_rnn_layers,bidirectional=True if self.mode =='bi' else False)
            self.hidden_to_pred = rslgym_module.MLP(
                hidden_to_pred_shape, activation_fn, hidden_size, priv_size, init_scale=1.0 / np.sqrt(2)
            )
            self.hidden_to_gate = rslgym_module.MLP(
                hidden_to_gate_shape, activation_fn, hidden_size, priv_size, init_scale=1.0 / np.sqrt(2)
            )
        ...
        if self.mode != "transformer":
            self.rnn = nn.GRU(encoder_input_dim , self.hidden_size, self.num_rnn_layers,bidirectional=True if self.mode =='bi' else False)
            self.attention=SelfAttention(2*self.hidden_size if self.mode == "bi" else self.hidden_size)
            if self.mode=="parallel":
                self.attention_exte=SelfAttention(2*self.hidden_size if self.mode == "bi" else self.hidden_size)
        ...
        if self.paradigm != "ordinal":
            ...
            [seq_length if not self.middle_only else 1 ,priv_size ], until=1e8, clip_threshold=10, is_training=is_training
        )
    ...
    def forward(self, latent, hidden=None):
        if latent.shape[-1] == 441 or latent.shape[-1] == 233:
            ...
            inner = torch.empty((sens.shape[0], sens.shape[1], 0), device=sens.device, dtype=sens.dtype)  # Empty tensor with zero size in the last dimension
    ...
    def convert_rnn_input(self, sens, inner):
        ...
        x = x.permute(1, 0, 2)  # Permute to get [sequence_length, batch_size, feature_size]
        return x
    ...
    def init_hidden(self, batch_size):
        ...
        """
        h0 = torch.zeros(self.num_rnn_layers if self.mode !='bi' else 2*self.num_rnn_layers, batch_size, self.hidden_size).to(self.device)
        return h0
    ...
    def calc_prediction_loss(self, output,  priv):
        ...
        if output.shape[1]!=priv.shape[1]:
            if self.middle_only and output.shape[1]==1:
                ...
                raise ValueError("Expected output and priv to have the same sequence length dimension")
    ...
    def get_unnormalized_recon(self, latent,hidden):
        if self.paradigm!= "ordinal":
            ...
            if self.discretization == "SID":
                ...
                t0 = torch.exp(np.log(alpha)+np.log(beta/alpha) * label.float() / self.ord_num)-self.shift
                t1 = torch.exp(np.log(alpha)+np.log(beta/alpha) * (label.float() + 1) / self.ord_num)-self.shift
            else:
...
class BeliefDecoderMLP(nn.Module):
    def __init__(
        ...
        self.priv_decoder = rslgym_module.MLP(
            priv_decoder_shape, activation_fn, self.latent_size, priv_size, init_scale=1.0 / np.sqrt(2)
        )

        if self.mode == "separate":
            self.separate_layers = nn.ModuleList([nn.Linear(priv_size, priv_size // 4) for _ in range(4)])
    ...
    def forward(self, latent):
        ...
        if self.mode == "separate":
            ...
            for i in range(4):
                priv_recon_separated.append(self.separate_layers[i](priv_recon[:, i :i + 1]))
...
class BeliefDecoderLightning(pl.LightningModule):
    def __init__(self, args, model_name="model", use_rnn=False, reset_hidden_each_epoch=True):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_rnn:
            ...
            if (self.reset_hidden_each_epoch ) or self.hidden_state is None:
                self.hidden_state = self.decoder.init_hidden(batch_size)
                # print(f"Reset hidden state for RNN:{self.model_name}")
            output, self.hidden_state = self.decoder(latent, self.hidden_state)  # Don't update the hidden state during validation
            loss = self.decoder.calc_prediction_loss(output, priv)
        else:
            loss = self.decoder.calc_prediction_loss(latent, priv)
