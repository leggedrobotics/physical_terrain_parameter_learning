        ...
        for i in range(len(boundaries) - 1):
            ...
            if use_rnn:
                ...
                for j in range(episode_start, episode_end, interval):
                    ...
                    stiff_data = sensors["stiff"][
                        j : j + sequence_length, selected_cols
                    ]
                    ...
                    data_points.append(
                        {
                            "input": torch.cat(
                                [
                                    policy_data[j].unsqueeze(0),
                                    exte_data[j].unsqueeze(0),
                                    hidden_data[j].unsqueeze(0),
                                ],
                                dim=-1,
                            ),
                            "output": torch.cat(
                                [fric_data[j].unsqueeze(0), stiff_data[j].unsqueeze(0)],
                                dim=-1,
                            ),
                        }
                    )
...
def train_and_evaluate(param: ParamCollection):
    ...
    for model_type in model_types:
        ...
        if SEPARATE_DATASET_MODE:
            train_data = combine_datasets_from_directory(
                train_data_directory, model_type, "train"
            )

            organized_train_data = organize_data(
                train_data, seq_length, use_rnn, overlapp
            )
        else:
            ...
            if latest_train_data_file:
                ...
                raise NoDataFileError(
                    "No training data file found in the specified directory."
                )
        ...
        if SEPARATE_DATASET_MODE:
            val_data = combine_datasets_from_directory(
                val_data_directory, model_type, "val"
            )
            ...
            if latest_val_data_file:
                ...
                organized_val_data = organize_data(
                    val_data, seq_length, use_rnn, overlapp
                )
            else:
                raise NoDataFileError(
                    "No validation data file found in the specified directory."
                )
        ...
        if mode == "train":
            ...
            if continue_training:
                ...
                if not latest_model_file:
                    raise ValueError(
                        f"No '{model_type}' model found in the specified directory for continuation."
                    )

                model = LightningWrapper.create_lightning(
            ...
            save_model(
                model, model_type, use_rnn, input_width, use_weight, hyperparameters
            )

        # Testing mode after training

        search_pattern = construct_search_pattern(
            use_rnn, rnn_mode, input_type, use_weight, model_type, paradigm=paradigm
        )
        latest_model_file = get_latest_file_in_directory(
            model_directory, pattern=search_pattern
        )
        ...
        if latest_model_file:
            ...
            if not model.decoder.prob:
                ...
                true_values, errors = collect_errors(
                    model, val_loader, use_rnn, reset_hidden_each_epoch
                )
                if SEPARATE_DATASET_MODE:
                    plot_error_histogram(
                        true_values,
                        errors,
                        0.1 if model_type == "fric" else 1,
                        0 if model_type == "fric" else 1,
                        1 if model_type == "fric" else 10,
                        model_type,
                        use_rnn,
                        input_width,
                        timestamp,
                        ratio=1.0 if model_type == "fric" else 0.1,
                ...
                true_values, errors, stds = collect_errors_prob(
                    model, val_loader, use_rnn, reset_hidden_each_epoch
                )
                plot_error_histogram_prob(
                    true_values,
                    errors,
                    stds,
                    0.1 if model_type == "fric" else 1,
